...
export default function Endpoint_RetrieveKey() {
  ...
  return (
    ...
    >
      ...
      <h3 style={{ marginBottom: '1rem', color: 'var(--krnl-primary)' }}>Endpoint: RetrieveKey</h3>
      <form onSubmit={handleSubmit}>
        <KrnlTextInput label="Secret Key ID" value={formData['id'] || ''} onChange={(v) => handleInputChange('id', v)} placeholder="e.g. gemini_key" />
        <KrnlButton type="submit" disabled={loading} label={loading ? 'Executing...' : 'Execute RetrieveKey'} icon="⚡" />
      </form>
      ...
      {response !== null && (
        <div style={{ marginTop: '1.5rem', borderTop: '1px solid var(--krnl-border-color)', paddingTop: '1rem' }}>
          ...
          <KrnlTile title="Decrypted Secret Value" value={String(response['value'] !== undefined ? response['value'] : response)} />
        </div>
      )}
    </div>
  );
