...
import { KrnlButton, KrnlJsonViewer } from '../../atoms/index.js';

export default function Endpoint_ListKeys() {
  ...
  return (
    ...
    >
      ...
      <h3 style={{ marginBottom: '1rem', color: 'var(--krnl-primary)' }}>Endpoint: ListKeys</h3>
      <form onSubmit={handleSubmit}>
        
        <KrnlButton type="submit" disabled={loading} label={loading ? 'Executing...' : 'Execute ListKeys'} icon="⚡" />
      </form>
      ...
      {response !== null && (
        <div style={{ marginTop: '1.5rem', borderTop: '1px solid var(--krnl-border-color)', paddingTop: '1rem' }}>
          ...
          <KrnlJsonViewer label="Stored Secrets Vault" data={response['keys'] !== undefined ? response['keys'] : response} />
        </div>
      )}
    </div>
  );
